...
def main():
    ...
    for plugin in root.iter('plugin'):
        ...
        elif 'robot_imu_plugin' in plugin.attrib.values():
            imu_plugin = plugin
    ...
    while True:
        ...
        # robot_file_path = tmp.joinpath(args.robot_name)

        # for f in os.listdir(tmp):
        #     file_path = tmp.joinpath(f)
        #     if file_path.is_file:
        #         count += 1

        # if count >= 10:
        #     node.get_logger().info('Count 10+ files, so sleep')
        #     time.sleep(10)
        #     continue
        # else:
        #     node.get_logger().info(f'Found less then 10 files, so create file: {robot_file_path}')
        #     robot_file_path.touch()
        ...
        if future.result() is not None:
            ...
            # node.get_logger().info(f'Finished creating node, remove file: {robot_file_path}')
            break
        else:
